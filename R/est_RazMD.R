#' @title Estimador de Razón en Muestreo Doble
#' @description Calcula las estimaciones de la media y el total poblacional, así como sus varianzas estimadas, utilizando un estimador de razón en un diseño de muestreo doble.
...
#' @details
#' En el muestreo doble para razón, se utiliza una primera muestra para estimar la media poblacional de la variable auxiliar, y una segunda muestra más pequeña para estimar la razón entre la variable de interés y la auxiliar.
#' @return Una lista con los siguientes componentes:
#' \itemize{
#'    \item \code{Estimación de la razón}
...
est_RazMD <- function(xp,y,x,N)
{
  ...
  varEstMedia <- (N-np)/(N*np)*var(y) + (np-n)/(np*n)*(var(y)+estR^2*var(x)-2*estR*cov(x,y))
  varEstTotal <- varEstMedia*N^2
  
  #devolvemos los valores
  return(list("Estimación de la razón"=estR,
